...
bool	is_valid_input(int argc, char **argv)
{
	...
	if (argc < 5 || argc > 6)
		return (msg(STR_ERR_USAGE, NULL, NULL), false);
	i = 0;
	while (argv[++i])
	{
		if (!contains_only_digits(argv[i]))
			return (msg(STR_ERR_INPUT_DIGIT, argv[i], NULL), false);
		if (i == 1 && !in_positive_range(argv[i], STR_MAX_PHILOS))
			return (msg(STR_ERR_INPUT_MAXPHILOS, argv[i], STR_MAX_PHILOS), false);
